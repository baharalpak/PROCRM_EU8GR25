Feature: CRM services feature.
  ...
  @wip
  Scenario Outline: Hr, Helpdesk and Marketing user can display 'My Courses'
  ...
  Scenario Outline: Hr, Helpdesk and Marketing user can display 'Grade Book'
    Given "<user>" user is on homepage
    When User clicks on the services tab.
    And  User clicks on the training tab.
  ...
  Scenario Outline: Hr, Helpdesk and Marketing user can edit their data under Student's Profile
    Given "<user>" user is on homepage
    When User clicks on the services tab.
    And  User clicks on the training tab.
